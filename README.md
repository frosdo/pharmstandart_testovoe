...
настроил конфиг `auditd.conf`, чтобы логи писалались в `/var/log/audit/secure`. сначала что файл не появлялся, но после генерации событий (попробовал зайти пару раз с неправильным паролем он создался.
