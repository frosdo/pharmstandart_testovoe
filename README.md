...
вбил команду `cat /etc/astra_version` — увидел 1.8.1. Проверил, что модуль PARSEC загружен (`lsmod | grep parsec`) — есть. Целостность пакетов проверил через `dpkg -V` — тишина, значит ок.
...
при установке PostgreSQL система просила DVD-диск, потому что в `sources.list` был прописан локальный реп. Решил закомментировать `cdrom` и добавить официальные репозитории Astra — заработало.
